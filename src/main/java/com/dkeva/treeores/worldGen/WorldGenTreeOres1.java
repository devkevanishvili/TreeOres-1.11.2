package com.dkeva.treeores.worldGen;

import com.dkeva.treeores.blocks.BlockTLeaves1;
import com.dkeva.treeores.blocks.BlockTLogs1;
import com.dkeva.treeores.blocks.BlockTSaplings1;
...
public class WorldGenTreeOres1 extends WorldGenAbstractTree {
    private static final IBlockState DEFAULT_TRUNK = TBlocks.TLogs1.getDefaultState().withProperty(BlockTLogs1.VARIANT,TV1.IRON);
    private static final IBlockState DEFAULT_LEAF = TBlocks.TLeaves1.getDefaultState().withProperty(BlockTLeaves1.VARIANT, TV1.IRON).withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));
    ...
    {
        this(doBlockNotify, 4, DEFAULT_TRUNK, DEFAULT_LEAF, false);
    }

    public WorldGenTreeOres1(boolean doBlockNotify, int mTreeHeight, IBlockState wood, IBlockState leaves, boolean vGrow)
    {
        super(doBlockNotify);
        this.minTreeHeight = mTreeHeight;
        this.metaWood = wood;
        this.metaLeaves = leaves;
        this.vinesGrow = vGrow;
    }
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= worldIn.getHeight())
        {
            ...
            else
            {
                ...
                if (state.getBlock().canSustainPlant(state, worldIn, position.down(), net.minecraft.util.EnumFacing.UP, (BlockTSaplings1)TBlocks.TSaplings1) && position.getY() < worldIn.getHeight() - i - 1)
                {
