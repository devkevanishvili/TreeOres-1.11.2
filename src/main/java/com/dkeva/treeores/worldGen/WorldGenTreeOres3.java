...
import com.dkeva.treeores.blocks.*;
...
public class WorldGenTreeOres3 extends WorldGenAbstractTree {
    private static final IBlockState DEFAULT_TRUNK = TBlocks.TLogs3.getDefaultState().withProperty(BlockTLogs3.VARIANT, TV3.GLOWSTONE);
    private static final IBlockState DEFAULT_LEAF = TBlocks.TLeaves3.getDefaultState().withProperty(BlockTLeaves3.VARIANT, TV3.GLOWSTONE).withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));
    ...
    public WorldGenTreeOres3(boolean doBlockNotify, int mTreeHeight, IBlockState wood, IBlockState leaves, boolean vGrow)
    {
        super(doBlockNotify);
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= worldIn.getHeight())
        {
            ...
            else
            {
                ...
                if (state.getBlock().canSustainPlant(state, worldIn, position.down(), EnumFacing.UP, (BlockTSaplings3)TBlocks.TSaplings3) && position.getY() < worldIn.getHeight() - i - 1)
                {
