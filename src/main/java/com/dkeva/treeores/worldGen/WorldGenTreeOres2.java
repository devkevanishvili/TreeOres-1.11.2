...
public class WorldGenTreeOres2 extends WorldGenAbstractTree {
    private static final IBlockState DEFAULT_TRUNK = TBlocks.TLogs2.getDefaultState().withProperty(BlockTLogs2.VARIANT, TV2.DIAMOND);
    private static final IBlockState DEFAULT_LEAF = TBlocks.TLeaves2.getDefaultState().withProperty(BlockTLeaves2.VARIANT, TV2.DIAMOND).withProperty(BlockLeaves.CHECK_DECAY, Boolean.valueOf(false));
    ...
    public WorldGenTreeOres2(boolean doBlockNotify, int mTreeHeight, IBlockState wood, IBlockState leaves, boolean vGrow)
    {
        super(doBlockNotify);
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position)
    {
        ...
        if (position.getY() >= 1 && position.getY() + i + 1 <= worldIn.getHeight())
        {
            ...
            else
            {
                ...
                if (state.getBlock().canSustainPlant(state, worldIn, position.down(), EnumFacing.UP, (BlockTSaplings2)TBlocks.TSaplings2) && position.getY() < worldIn.getHeight() - i - 1)
                {
